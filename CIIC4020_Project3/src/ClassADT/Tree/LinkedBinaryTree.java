...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> implements Cloneable {
	...
	// Creating a CLONE
		...
		private void cloneSubtree(Position<E> rThis, LinkedBinaryTree<E> other,
				Position<E> rOther) {
			Position<E> hold = rThis;
			for (Position<E> pThis : children(rThis)) { 
				if(pThis.equals(this.left(hold))) {
					Position<E> nextOther = other.addLeft(rOther, pThis.getElement());
					cloneSubtree(pThis, other, nextOther);
				}else if(pThis.equals(this.right(hold))) {
					Position<E> nextOther = other.addRight(rOther, pThis.getElement());
					cloneSubtree(pThis,other,nextOther);
				}
			}
		}
